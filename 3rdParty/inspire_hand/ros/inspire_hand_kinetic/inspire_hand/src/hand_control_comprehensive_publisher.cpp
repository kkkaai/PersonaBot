...
void setangles(serial::Serial *port)
{
    int add = 1486;
    ...
    output.push_back(angles_set[0] & 0xff);
    output.push_back((angles_set[0] >> 8) & 0xff);
    output.push_back(angles_set[1] & 0xff);
    output.push_back((angles_set[1] >> 8) & 0xff);
    output.push_back(angles_set[2] & 0xff);
    output.push_back((angles_set[2] >> 8) & 0xff);
    output.push_back(angles_set[3] & 0xff);
    output.push_back((angles_set[3] >> 8) & 0xff);
    output.push_back(angles_set[4] & 0xff);
    output.push_back((angles_set[4] >> 8) & 0xff);
    output.push_back(angles_set[5] & 0xff);
    output.push_back((angles_set[5] >> 8) & 0xff);
...
int main(int argc, char** argv){
    ...
    ros::init(argc, argv, "hand_control_comprehensive_publisher");
    ros::NodeHandle nh;

    ros::Publisher pub = nh.advertise<inspire_hand::status>("/status_topic", 1);

    ros::Rate loop_rate(20); // unit: hz
    ...
    while(ros::ok()){
        ...
        {
            angles_set[0] += direction*120;
            angles_set[1] += direction*120;
            angles_set[2] += direction*120;
            angles_set[3] += direction*120;
            if(angles_set[0] <= 1)
            {
                direction = 1;
            }
            else if(angles_set[0] >= 999)
            {
                direction = -1;
            }
            static int get = 1;
